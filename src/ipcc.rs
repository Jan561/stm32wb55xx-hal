...
impl Ipcc {
    ...
    pub(crate) fn new(rb: IPCC, rec: &mut rec::IPCC) -> Self {
        rec.enable();

        rb.c1cr.modify(|_, w| w.rxoie().set_bit().txfie().set_bit());
...
pub trait IpccExt {
    fn tl_init(self, rec: &mut rec::IPCC) -> (TlMbox, Ipcc);
}

impl IpccExt for IPCC {
    fn tl_init(self, rec: &mut rec::IPCC) -> (TlMbox, Ipcc) {
        TlMbox::tl_init(self, rec)
    }
}
